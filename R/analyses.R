  ...
  output %>%
    mutate(
      species = species %>%
        ...
        str_remove("^Candidatus[ _]+") %>% # remove leading Candidatus_ or Candidatus␠
        str_replace_all("_", " ") %>% # underscores -> spaces
        str_squish(), # trim + collapse multiple spaces

      mutation_name = str_c(AA_pos_Ecoli, AA_mutation, sep = "_"),
      genus = species %>%
        word(1) %>%
        str_to_title(),
      ...
      mutation_category = case_when(
        AA_mutation == AA_target ~ "present",
        n_possible == 0L ~ "impossible",
        TRUE ~ "possible"
      )
    )
}
...
compare_gene_copies <- function(filtered_output, target_sequences, reference_Ecoli) {
  ...
  multiseq_stats <- future_lapply(1:length(multicopy_species), function(i) {
    ...
    if (length(seq_names) > 2L) {
      ...
      coords1 <- ALJEbinf::getCoordinates(
        target_sequences[[seq_names[1]]],
        reference_Ecoli
      )
      coords2 <- ALJEbinf::getCoordinates(
        target_sequences[[seq_names[2]]],
        reference_Ecoli
      )
      ...
      output <- list(
        accession_number = multicopy_species[i],
        ...
        core_dist = as.double(pwalign::stringDist(
          c(
            Biostrings::AAStringSet(core_target1),
...
get_species_output <- function(output) {
  species_output <- output |>
    ...
    summarise(
      resistance = dplyr::first(resistance),
...
get_theoretical_evolvabilities <- function(output) {
  ...
  possibilities <- expand_grid(AA_pos_Ecoli,
    ...
    select(-hamming) |>
    mutate(
      AA_from = Biostrings::GENETIC_CODE[codon_from],
      AA_to = Biostrings::GENETIC_CODE[codon_to]
    ) |>
    mutate(
      AA_mut_name_from = paste(AA_pos_Ecoli, AA_from, sep = "_"),
      AA_mut_name_to = paste(AA_pos_Ecoli, AA_to, sep = "_")
    ) |>
    mutate(
      resistant_from = AA_mut_name_from %in% mutation_list$mut_name,
  ...
  evolvabilitiesI <- possibilities |>
    ...
    group_by(AA_pos_Ecoli) |>
    summarise(
      min_evolvabilityI = min(evolvabilityI),
      max_evolvabilityI = max(evolvabilityI)
    )
  ...
  evolvabilitiesII <- possibilities |>
    ...
    group_by(AA_pos_Ecoli) |>
    summarise(
      min_evolvabilityII = min(evolvabilityII),
...
get_conservation <- function(target_sequences,
  ...
  hamming_rnd <- matrix(NA,
    nrow = n_rnd, ncol = l,
    dimnames = list(
      paste0(
        names(target_sequences)[ijs$i],
        ":::",
  ...
  means <- tibble(
    hamming_Ecoli = colMeans(hamming_Ecoli, na.rm = TRUE),
